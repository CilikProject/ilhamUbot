...
async def autopilot():
    ...
    try:
        r = await bot(
            CreateChannelRequest(
                title="Lɪʙɪ-Usᴇʀʙᴏᴛ Lᴏɢs",
                about="» Group log Created by: Libi-Userbot\n\n» Support : @janvilion",
                megagroup=True,
            ),
        )
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo = await download_file(
            "https://telegra.ph/file/c64a115ba83a4749050e5.jpg", "photoyins.jpg"
        )
...
async def autobot():
    ...
    if who.username:
        ...
        username = f"Libi{(str(who.id))[5:]}bot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = f"Libi{(str(who.id))[6:]}{str(ran)}bot"
        await bot.send_message(bf, username)
        await asyncio.sleep(1)
